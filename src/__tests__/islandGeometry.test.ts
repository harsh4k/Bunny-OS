...
describe("islandGeometry", () => {
  it("derives window size from pill + gutters + border", () => {
    ...
    expect(ISLAND_WINDOW).toEqual({ width: 276, height: 50 });
  });
  ...
  it("applies CSS vars used by VoicePill", () => {
    ...
    expect(style.getPropertyValue("--island-pad-y")).toBe("6px");
    expect(style.getPropertyValue("--island-window-w")).toBe("276px");
    expect(style.getPropertyValue("--island-window-h")).toBe("50px");
  });
});
